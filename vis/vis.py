...
def make_heatmaps(image, heatmaps):
    ...
    # std_color1 = [255, 230, 20]
    # std_color2 = [20, 247, 255]

    # std_color1 = [20, 247, 255]
    # std_color2 = [138, 60, 224] * 1.2

    # std_color1 = [20, 247, 255]
    # k2 = 1.2
    # std_color2 = [255 * k2, 230 * k2, 20 * k2]

    # std_color1 = [20, 247, 255]
    # k2 = 1.2
    # std_color2 = [138 * k2, 60 * k2, 224 * k2]

    # k1 = 1.4
    # std_color1 = [51 * k1, 255 * k1, 102 * k1]
    # k2 = 1.7
    # std_color2 = [66 * k2, 64 * k2, 203 * k2]

    # k1 = 1.7
    # std_color1 = [66 * k1, 64 * k1, 203 * k1]
    # k2 = 1.2
    # std_color2 = [255 * k2, 230 * k2, 20 * k2]

    k1 = 1.5
    std_color1 = [0 * k1, 225 * k1, 255 * k1]
    k2 = 1.2
    std_color2 = [255 * k2, 255 * k2, 0 * k2]
    ...
    for j in range(4):
        ...
        '''
        heatmap1 = heatmaps[0, :, :]
        hard_map1 = (heatmap1 >= 0.2) * 1
        colored_heatmap1 = np.stack([hard_map1 * std_color1[0], hard_map1 * std_color1[1], hard_map1 * std_color1[2]], axis=2)
        heatmap2 = heatmaps[1, :, :]
        hard_map2 = (heatmap2 >= 0.2) * 1
        colored_heatmap2 = np.stack([hard_map2 * std_color2[0], hard_map2 * std_color2[1], hard_map2 * std_color2[2]], axis=2)
        colored_heatmap = colored_heatmap1 + colored_heatmap2
        '''
        heatmap = heatmaps[j, :, :]
        hard_map = (heatmap >= 0.1) * 1
        # hard_map = heatmap
        colored_heatmap = np.stack([hard_map * std_color1[0], hard_map * std_color1[1], hard_map * std_color1[2]], axis=2)

        background = 1-(colored_heatmap==0)
        background=background.astype(np.float)
        for ch in range(3):
            colored_heatmap[:, :, ch] = 1.2 * gaussian_filter(colored_heatmap[:, :, ch], sigma=3)
        background = gaussian_filter(background, sigma=2)
        image_fused = 0.7 * colored_heatmap + 0.3 * image_resized
        # image_fused = cv2.addWeighted(colored_heatmap, 0.5, image_resized, 0.5, 128, dtype=cv2.CV_64F)
        # background = background == 0
        # image_fused[background] = image_resized[background]
        image_fused = image_resized * (1- background) + image_fused * (background)
        image_fused = image_fused.clip(0, 255)
